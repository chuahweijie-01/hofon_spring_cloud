...
exports.registerUser = (userInfo, companyOfficialId) => {
    ...
    return connectionPool.getConnection()
        ...
        .then(([rows, field]) => {
            if (rows.length) throw new Error('帳戶已被使用，請使用新的郵箱進行注冊。');
            ...
            FROM
                companydb.company
            WHERE
                company_official_id = ?`, [companyOfficialId]);
        })
        .then(([rows, field]) => {
            if (rows.length) {
            ...
            else throw new Error('該公司并未注冊雲端系統。');
        })
        .then((result) => {
            ...
            INSERT INTO
                userdb.user_company
                (user_id, company_id)
            VALUES (?,?)`, [userInfo.user_id, companyId]);
            else throw new Error('帳號注冊失敗');
        })
        .then((result) => {
