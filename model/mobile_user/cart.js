...
exports.cart_products = (user_id, company_id) => {
    return connectionPool.getConnection()
        .then((connection) => {
            return connection.query(`SELECT product.product_id, product.product_name, product.product_price, cart_product.quantity, SUM(product.product_price * cart_product.quantity) AS total_price
                                     ...
                                     WHERE cart.user_id = ? AND cart.company_id = ?
                                     GROUP BY product.product_id`, [user_id, company_id])
                .then(([rows, field]) => {
                    return (rows);
                })
                .finally(() => {
