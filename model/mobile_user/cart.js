    ...
    return connectionPool.getConnection()
        .then((connect) => {
            ...
            return connection.query(`SELECT product.product_id, product.product_name, product.product_price, product.product_member_price, cart_product.quantity, image.image_path,
                                     ...
                                     WHERE cart.user_id = ? AND company.company_id = ? GROUP BY product.product_id`, [userId, companyId]);
        })
        .then(([rows, field]) => {
            return (rows);
        })
...
exports.deleteCart = (userId, companyId) => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            connection = connect;
            return connection.query(`DELETE FROM userdb.cart WHERE user_id = ? AND company_id = ? `, [userId, companyId]);
        })
        .then((result) => {
