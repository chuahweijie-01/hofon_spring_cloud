...
exports.deleteCartItem = (cartId, orderId) => {
    ...
    return connectionPool.getConnection()
        ...
        .then(([rows, field]) => {
            ...
            WHERE
                product_id IN (?)
                AND cart_id = ?`, [productArray, cartId])
        })
        .then((result) => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            ...
            SET
                ...
                trade_number = ?
            WHERE
                order_id = ?`, [paymentDate, tradeDate, tradeNo, orderId])
        })
        .then((result) => {
            if (result[0].info.match('Changed: 1')) return connection.query(`
            SELECT
                product_id,
                quantity
            FROM
        ...
        .then(([rows, field]) => {
            for (var i = 0; i < rows.length; i++) {
                ...
                UPDATE
                    productdb.product
                SET
                    product_stock = GREATEST(product_stock - ?, 0),
                    total_sales = (total_sales + ?)
                WHERE
                    product_id = ?`, [rows[i].quantity, rows[i].quantity, rows[i].product_id])
                    .then((result) => {
                        // Update continue ...
                    })
            }
