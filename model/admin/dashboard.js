...
exports.getOrderSummary = (companyId) => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            ...
            (SELECT COUNT(*) AS total FROM orderdb.order WHERE order_status = 0 AND company_id = ?) AS pending,
            (SELECT COUNT(*) AS total FROM orderdb.order WHERE order_status = 1 AND company_id = ?) AS done,
            (SELECT COUNT(*) AS total FROM orderdb.order WHERE order_status = 2 OR order_status = 3 AND company_id = ?) AS other,
            (SELECT COUNT(*) AS total FROM orderdb.order WHERE company_id = ?) AS total
...
exports.getPanelBoardSummary = (companyId) => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            ...
            (SELECT COUNT(*)FROM userdb.user AS user JOIN userdb.user_company AS user_company USING (user_id) WHERE user_company.company_id = ?) AS totalUser,
            (SELECT IFNULL(SUM(order_total_item), 0) FROM orderdb.order WHERE company_id = ?) AS totalSales,
            (SELECT IFNULL(SUM(order_total_item), 0) FROM orderdb.order WHERE company_id = ? AND MONTH(trade_date) = MONTH(CURRENT_DATE())) AS currentMonthSales`, [companyId, companyId, companyId, companyId, companyId]);
        })
        .then(([rows, field]) => {
            return rows;
        })
...
exports.getUserRecord = (companyId) => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            ...
            JOIN userdb.user_company AS user_company USING (user_id)
...
exports.getDatabaseCondition = () => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            connection = connect;
            return connection.query(`SELECT TABLE_SCHEMA 'db', COUNT(*) 'tb', SUM(data_length + index_length) 'b', ROUND(SUM(data_length + index_length) / 1024 / 1024, 2) 'Mb'
            FROM information_schema.tables WHERE table_schema IN ('analysisdb', 'companydb', 'orderdb', 'productdb', 'userdb') GROUP BY table_schema`)
        })
        .then(([rows, field]) => {
            return rows;
        })
...
exports.getAdminPanelBoardSummary = () => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            connection = connect;
            return connection.query(`SELECT 
            (SELECT COUNT(*) FROM companydb.company WHERE company_id <> '1') AS totalCompany,
            (SELECT COUNT(*) FROM userdb.user) AS totalUser,
            (SELECT COUNT(*) FROM companydb.admin WHERE admin_role = 0) AS totalClient`)
        })
        .then(([rows, field]) => {
            return rows;
        })
...
exports.getOverallOrderReport = () => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            connection = connect;
            return connection.query(`SELECT order_id, YEAR(trade_date) AS tradeYear, MONTH(trade_date) AS tradeMonth, DAY(trade_date) AS tradeDate, COUNT(*) AS totalOrder, SUM(order_total_item) AS totalItem
            FROM orderdb.order WHERE MONTH(trade_date) = MONTH(CURRENT_DATE()) AND YEAR(trade_date) = YEAR(CURRENT_DATE()) GROUP BY tradeDate`);
        })
        .then(([rows, filed]) => {
            return rows;
        })
...
exports.getOverallOrderStatistic = (month, year) => {
    ...
    return connectionPool.getConnection()
        .then((connect) => {
            connection = connect;
            return connection.query(`SELECT order_id, YEAR(trade_date) AS tradeYear, MONTH(trade_date) AS tradeMonth, DAY(trade_date) AS tradeDate, COUNT(*) AS totalOrder, SUM(order_total_item) AS totalItem
            FROM orderdb.order WHERE MONTH(trade_date) = ? AND YEAR(trade_date) = ? GROUP BY tradeDate`, [month, year]);
        })
        .then(([rows, filed]) => {
            return rows;
        })
