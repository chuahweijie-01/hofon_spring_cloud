    ...
    return connectionPool.getConnection()
        ...
        .then(([rows, field]) => {
            ...
            SELECT
                ...
                zone.zone_charge,
                COUNT(*) AS total_city,
                DATE_FORMAT(zone.last_update, '%d-%c-%Y %H:%i:%s') AS last_update
            FROM
...
exports.addNewZone = (zoneInfo, cityId) => {
    var connection;
    var zoneId = zoneInfo.zone_id;
    var zoneName = zoneInfo.zone_name;
    return connectionPool.getConnection()
        .then((connect) => {
            connection = connect;
            return connection.query(`
        ...
        .then((result) => {
            if (result[0].affectedRows >= 1) return (`已新增${zoneName}區域`);
            else throw new Error(`資料新增失敗`);
        })
        .catch((err) => {
            console.error(err);
...
exports.updateZone = (zoneInfo, zoneId, cityId) => {
    var connection;
    return connectionPool.getConnection()
        .then((connect) => {
            connection = connect;
